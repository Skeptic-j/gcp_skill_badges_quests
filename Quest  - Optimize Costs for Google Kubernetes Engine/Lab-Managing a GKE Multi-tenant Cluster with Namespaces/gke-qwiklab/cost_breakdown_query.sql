WITH
  ...
  FROM
    `qwiklabs-gcp-00-ac4421112a1d.billing_dataset.gcp_billing_export_v1_xxxx`
  WHERE
    usage_start_time >= TIMESTAMP("2020-10-26")
  GROUP BY
    project.id,
    sku.id,
    usage.unit ),
  ...
  SELECT
    ...
    billing_table.rate
  FROM
    `qwiklabs-gcp-00-ac4421112a1d.cluster_dataset.gke_cluster_resource_usage` AS resource_usage
  INNER JOIN
    billing_table
  ON
  ...
  consumption_based_cost_allocation AS (
      WITH
        ...
        SELECT
          ...
          billing_table.rate
        FROM
          `qwiklabs-gcp-00-ac4421112a1d.cluster_dataset.gke_cluster_resource_consumption` AS resource_usage
        INNER JOIN
          billing_table
        ON
          resource_usage.sku_id = billing_table.sku_id
